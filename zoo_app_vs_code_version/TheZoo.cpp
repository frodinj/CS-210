...
void AddAnimal() {
	...
	cout << "FIXME: AddAnimal()" << endl;       // testing line - remove from final version
    ...
    try {
        ...
        cin >> t_type;
        for (int i = 0; i < 1; i++) {
            t_type.at(i)= toupper(t_type.at(i));           // allows user to enter 'oviparous' or 'mammal' but converts it
        }
...
void RemoveAnimal() {
	...
	try {
        cout << "FIXME: RemoveAnimal()" << endl;                    // testing code - remove from final version
        ...
        cout << "RESULT: " << result << endl;           // testing code - remove from final version
...
/* This load from file function fetches data from an external file in a specific format per the notes in § 3.b
...
void LoadDataFromFile() {
	...
	cout << "FIXME: LoadDataFrom()" << endl;        // testing code - remove from final version
    ifstream inFS; // use file stream
    cout << "Opening File: FIXME - Find file name from Java Function" << endl;      // testing code - remove from final version
    inFS.open("C:\\Users\\deade\\projects\\zoo_app\\animals.txt");  // opening java file for data retrieval
    //AnimalList.resize(11);
    if (!inFS.is_open()) {          // error handling for file stream
        cout << "Could not open File. Try again." << endl;
    }
    // loop to read file
    cout <<  "VECTOR SIZE BEFORE LOAD: " << AnimalList.size() << endl;
    while (!inFS.eof()) {
        getline(inFS, input);   // gets full line from file at a time
        fileOutput += input;    // testing code - remove from final version
        fileOutput.push_back('\n'); // testing code - remove from final version
        cout << input << endl;      // testing code - remove from final version
        // loop to generate animal items and populate the <Animal*> vector
        for (int i = 0; i < 1; i++) {       // adjust param "/54" to match java file before final submission
    ...
    cout <<  "VECTOR SIZE AFTER LOAD: " << AnimalList.size() << endl;       // testing code - remove from final version
    cout << "Load Complete." << endl;    // message per instructions
    cout << fileOutput << endl;         // testing code - remove from final version
}

void SaveDataToFile() {
    char saveChar = 'a';
	cout << "FIXME: SaveDataToFile()" << endl;      // testing code - remove from final version
    ofstream outFS;         // variable to output data
    outFS.open("C:\\Users\\deade\\projects\\zoo_app\\animals2.txt");
    ...
    if (saveChar == 'y' || saveChar == 'Y') {
        ...
        /*for (const auto &e : AnimalList) {
            outFS << e* << endl;
            // end line after each animal is fully written out
            outFS << endl;
        }*/
    }
    else {
...
void DisplayAnimalData() {
	cout << "FIXME: create display function per requirement 5 of sheet" << endl;
	for (int i = 0; i < 102; i++) {
